...
open class RealmWaveformView: UIView {
    /// A delegate to accept progress reporting
    /*@IBInspectable*/ open weak var delegate: RealmWaveformViewDelegate?
    ...
    /*@IBInspectable*/ open var audioURL: URL? {
        didSet {
          print("loading for url:", audioURL?.absoluteString)
            guard let audioURL = audioURL else {
                NSLog("RealmWaveformView received nil audioURL")
                audioContext = nil
                return
            }
            ...
            FDAudioContext.load(fromAudioURL: audioURL) { audioContext in
                DispatchQueue.main.async {
                    ...
                    if audioContext == nil {
                        NSLog("RealmWaveformView failed to load URL: \(audioURL)")
                    }
    ...
    //TODO: make this public after reconciling RealmWaveformView.WaveformType and FDWaveformType
    enum WaveformType {
...
extension RealmWaveformView: UIGestureRecognizerDelegate {
...
/// To receive progress updates from RealmWaveformView
@objc public protocol RealmWaveformViewDelegate: NSObjectProtocol {
    /// Rendering will begin
    @objc optional func waveformViewWillRender(_ waveformView: RealmWaveformView)

    /// Rendering did complete
    @objc optional func waveformViewDidRender(_ waveformView: RealmWaveformView)

    /// An audio file will be loaded
    @objc optional func waveformViewWillLoad(_ waveformView: RealmWaveformView)

    /// An audio file was loaded
    @objc optional func waveformViewDidLoad(_ waveformView: RealmWaveformView)

    /// The panning gesture began
    @objc optional func waveformDidBeginPanning(_ waveformView: RealmWaveformView)

    /// The panning gesture ended
    @objc optional func waveformDidEndPanning(_ waveformView: RealmWaveformView)

    /// The scrubbing gesture ended
    @objc optional func waveformDidEndScrubbing(_ waveformView: RealmWaveformView)
}
