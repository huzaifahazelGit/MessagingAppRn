...
const PhotographyTabBtn = () => {
  const me:any = useMe();
    const navigation:any = useNavigation()
    const [loading,setLoading]= useState(true)
    const [imageLoading,setImageLoading]= useState(false)
    const [photographyData, setPhotographyData] = useState([]);
    const [photographyFeatureData, setPhotographyFetureData] = useState([]);
    const [photographyPopularData ,setPhotographyPopularData] = useState([])

    useFocusEffect(
      useCallback(() => {
        fetchMarkePhotoGraphyData()
      }, [])
    );  
    const fetchMarkePhotoGraphyData = async () => {      
      try {
       ...
       if (!querySnapshot.empty) {
          ...
          const photography = data.filter((item:any) => item?.category === 'PHOTOGRAPHY' && item?.isMarkSold === false);
          const photography_data = photography.filter((item:any) => item?.category === 'PHOTOGRAPHY' && item?.isEnabledFeature === false && item?.isBookMark === false );
          const popular_photography = photography.filter((item:any) =>  item?.isBookMark === true && item?.isEnabledFeature === false);        
          const feature_photography = photography.filter((item:any) => item?.isEnabledFeature === true);
          setPhotographyData(photography_data);
          setPhotographyFetureData(feature_photography)
          setPhotographyPopularData(popular_photography)
        } else {
          ...
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                fetchMarkePhotoGraphyData(); // Refresh the data to reflect the changes
              } catch (error) {
  ...
  return (
    <>
        <View style={styles.popularView}>
          ...
          ) : photographyPopularData.length === 0 ? (
          ...
          <ScrollView
            ...
            style={{ flex: 1 }}
          >
              {photographyPopularData.map((item, index) => (
                <Pressable 
        ...
        <View style={styles.featureView}>
          ...
          ) : photographyFeatureData.length === 0 ? (
            <View style={{ height:170,justifyContent:"center", }}>
              <AppText center size={15} color={"white"}>No Data found</AppText>
            </View>
          ) : (
          <ScrollView
            ...
            style={{ flex: 1 }}
          >
              {photographyFeatureData.map((item, index) => (
                <Pressable 
                onPress={() => navigation.navigate("ListingPage", { item })}
                key={index}
                >
...
export default PhotographyTabBtn
