...
const ArtTabBtn = () => {
    ...
    const [artPopularData, setArtPopularData] = useState([]);
    const [artFeatureData, setArtFeatureData] = useState([]);
    useFocusEffect(
      useCallback(() => {
        fetchMarketArtData()
      }, [])
    );  
    const fetchMarketArtData = async () => {      
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          const art = data.filter((item:any) => item?.category === 'Art' && item?.isMarkSold === false);
          const art_data = art.filter((item:any) => item?.category === 'Art' && item?.isEnabledFeature === false && item?.isBookMark === false) ;
          const popular_art = art.filter((item:any) =>  item?.isBookMark === true && item?.isEnabledFeature === false);     
          const feature_Art = art.filter((item:any) => item?.isEnabledFeature === true);
          setArtData(art_data);
          setArtFeatureData(feature_Art)
          setArtPopularData(popular_art)
        } else {
          ...
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                fetchMarketArtData(); // Refresh the data to reflect the changes
              } catch (error) {
