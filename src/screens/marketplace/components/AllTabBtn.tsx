...
const AllTabBtn = () => {
    ...
    useFocusEffect(
      useCallback(() => {
        // showToast('Buying/Selling Data is showing');
        fetchMarketAllData()

      }, [])
    );  
    const fetchMarketAllData = async () => {      
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          const all = data.filter((item:any) =>  item?.isMarkSold === false);
          ...
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                fetchMarketAllData(); // Refresh the data to reflect the changes
              } catch (error) {
